...
>
> **GitHub:**
> - ใช้ custom scalar `URI` แทน `String` สำหรับ URL ([GitHub GraphQL scalar reference](https://docs.github.com/en/graphql/reference/scalars))
> - `DateTime` scalar บังคับ ISO 8601 format — ไม่ต้องเดาว่า format คืออะไร
>
> **Shopify:**
> - `CurrencyCode` enum ครอบคลุมสกุลเงินตาม ISO 4217 ทั้งหมดที่ store รองรับ ([Shopify `CurrencyCode`](https://shopify.dev/docs/api/admin-graphql/latest/enums/currencycode))
> - enum สำหรับ order financial status เช่น `OrderDisplayFinancialStatus` ทำให้ client code เป็น exhaustive และ type-safe ([Shopify `OrderDisplayFinancialStatus`](https://shopify.dev/docs/api/admin-graphql/latest/enums/orderdisplayfinancialstatus))
