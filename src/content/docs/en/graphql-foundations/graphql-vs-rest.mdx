...
**Reaching for GraphQL because of hype, not a problem**

Symptoms:
- an API has 5 clear endpoints, but the team switches to GraphQL because it is "modern"
- the team spends time setting up GraphQL instead of shipping features

**Assuming GraphQL is always faster than REST**

Symptoms:
- no DataLoader — N+1 makes GraphQL much slower than REST
- no query depth limit — a client sends a deeply nested query and overloads the server

> 💡 Real-world examples
>
> **Stripe:**
> - Stripe's main public API is still REST today, not GraphQL ([Stripe API reference](https://docs.stripe.com/api))
> - proof that REST still fits a public API that needs HTTP caching, easy-to-reason-about versioning, and broad client-library support
>
> **Airbnb:**
