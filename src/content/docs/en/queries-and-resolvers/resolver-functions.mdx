...
**Heavy business logic in the resolver**

Symptoms:
- a resolver mixes validation, authorization, database queries, and transformation in one function
- hard to test because of all the side effects
- Fix: keep resolvers to 1-2 lines that call a service or repository

**No DataLoader for a relationship**

Symptoms:
- `Post.author`'s resolver runs `db.findUser(post.authorId)` for every post
- 100 posts means 100 database queries (N+1)
- Fix: use DataLoader to batch them into a single request

> 💡 Real-world examples
>
> **GraphQL Yoga:**
> - a spec-compliant GraphQL server that does not tie resolvers to any particular data source — one resolver can call a database, a REST API, or a cache ([Yoga docs](https://the-guild.dev/graphql/yoga-server))
>
> **Prisma:**
> - a popular ORM for database-backed resolvers ([Prisma Client docs](https://www.prisma.io/docs/orm/prisma-client))
> - `prisma.user.findUnique()` inside a resolver keeps the code short and type-safe
