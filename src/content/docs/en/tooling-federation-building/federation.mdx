...
**Adopting federation before there are multiple teams**

Symptoms:
- one team owns every subgraph
- more complex than a monolithic schema, with none of the benefit
- Fix: start with a monolithic schema, and federate once teams are genuinely separate

**Circular entity references**

Symptoms:
- the `User` subgraph references `Order`, and the `Order` subgraph references `User`, circularly
- the gateway has to make redundant round trips to resolve them
- Fix: design clear entity ownership — decide who owns whom

> 💡 Real-world examples
>
> **Airbnb:**
> - uses Apollo Federation for its internal GraphQL platform across teams ([Airbnb Engineering: Reconciling GraphQL and Thrift](https://medium.com/airbnb-engineering/reconciling-graphql-and-thrift-at-airbnb-a97e8d290712))
> - a schema gateway combines each team's subgraph for mobile and web clients
>
> **Netflix:**
> - federated GraphQL supports 70+ Domain Graph Services (DGSes) and hundreds of developers working independently ([Netflix TechBlog: GraphQL Federation part 1](https://netflixtechblog.com/how-netflix-scales-its-api-with-graphql-federation-part-1-ae3557c187e2), [part 2](https://netflixtechblog.com/how-netflix-scales-its-api-with-graphql-federation-part-2-bbe71aaec44a))
> - a schema registry catches breaking changes before deploy
