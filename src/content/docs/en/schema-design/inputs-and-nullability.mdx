...
**One input type shared between create and update**

Symptoms:
- `BookInput` is used by both `createBook` and `updateBook`
- create needs `title!` but update needs `title?`
- Fix: split it into `CreateBookInput` and `UpdateBookInput`

**Making every field non-null**

Symptoms:
- `UpdateBookInput { title: String!, authorId: ID!, pageCount: Int! }`
- the client must send every field even to update just one
- Fix: a partial update should use nullable fields, where "not sent" means "unchanged"

> 💡 Real-world examples
>
> **Relay Convention:**
> - the Relay Input Object Mutations Specification requires every mutation to take a single `input: XxxInput!` argument ([the spec](https://chentsulin.github.io/relay/graphql/mutations.htm))
> - this lets the schema evolve without changing the mutation's signature
>
> **GitHub:**
> - uses the naming convention `CreateIssueInput`, `UpdateIssueInput`, `CloseIssueInput` ([GitHub GraphQL mutation reference](https://docs.github.com/en/graphql/reference/mutations))
> - each operation has its own dedicated input type — none are shared
