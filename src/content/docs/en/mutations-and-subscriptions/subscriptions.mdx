...
**Using a subscription for everything instead of a query**

Symptoms:
- subscribing to `userProfile` to watch data that barely ever changes
- the server carries far more open connections than it needs to
- a plain query would be faster and cacheable

**Not filtering events by argument**

Symptoms:
- `subscription { reviewAdded }` delivers every review for every track, not just the one the client cares about
- the client has to filter client-side every time — needless extra traffic
- Fix: the resolver should take an argument like `trackId` to scope the subscription from the start

> 💡 Real-world examples
>
> **AWS AppSync:**
> - subscriptions accept arguments directly, e.g. `updatedPost(id: ID!, author: String)`, and the client subscribes with the values it wants ([AWS AppSync real-time subscriptions](https://docs.aws.amazon.com/appsync/latest/devguide/aws-appsync-real-time-data.html)), so it never receives events it doesn't care about
>
> **Hasura:**
> - a live-query subscription only emits a new result when the subscribed data actually changes (insert/update/delete) ([Hasura subscriptions docs](https://hasura.io/docs/2.0/subscriptions/overview/))
> - filtering through arguments/`where` on the subscription query itself scopes it to exactly the data a client cares about
